...
class MessageDeduplicator:
    ...
    def check_only(self, msg_id: str) -> bool:
        ...
        Thread-safe via ``self._lock`` (same lock as ``is_duplicate``).
        """
        if not msg_id:
            return False
        now = time.time()
        with self._lock:
            ...
            if len(self._seen) > self._max_size * 3:
                cutoff = now - self._ttl
                self._seen = {k: v for k, v in self._seen.items() if v > cutoff}
                # Hard cap: when every entry is still fresh, keep newest entries
                # so dict size doesn't stay at ~3× max_size under sustained traffic.
                if len(self._seen) > self._max_size:
                    newest = sorted(
                        self._seen.items(),
                        key=lambda item: item[1],
    ...
    def mark_seen(self, msg_id: str) -> None:
        """Record *msg_id* as seen. Thread-safe via ``self._lock``."""
        if not msg_id:
            return
        with self._lock:
    ...
    def clear(self):
        """Clear all tracked messages. Thread-safe."""
        with self._lock:
            self._seen.clear()
